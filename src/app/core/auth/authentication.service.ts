...
export class AuthenticationService implements AuthService {
	...
	public login(credential: Credential): Observable<any> {
		...
		return this.http.post<AccessData>(this.API_URL + this.API_ENDPOINT_LOGIN, body ).pipe(
			map((result: any) => {
				if (result instanceof Array) {
					return result.pop();
				}
				result['accessToken'] = result.authHeader;
				result['refreshToken'] = result.authHeader;
				return result;
			}),
			tap(this.saveAccessData.bind(this)),
